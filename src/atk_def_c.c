...
void send_atk_c(snavy_t const *navy, int current_hit, const int coordsxy[2],
    char const *pos)
{
    ...
    if (connection.last_hit_res == 0) {
        my_printf("hit\n\n");
        navy->s_map2d[coordsxy[1]][coordsxy[0]] = 'x';
    } else {
        my_printf("missed\n\n");
        navy->s_map2d[coordsxy[1]][coordsxy[0]] = 'o';
    }
}
...
void attack_c(snavy_t *navy, int current_hit)
{
    ...
    send_atk_c(navy, current_hit, coords, pos);
}
...
void defend_c(snavy_t *navy, int current_line)
{
    ...
    if (navy->c_map2d[ypos][xpos] != '.' && navy->c_map2d[ypos][xpos] != 'o') {
        my_printf("%s: hit\n\n", connection.last_get_string);
        navy->c_map2d[ypos][xpos] = 'x';
        send_packet("1100000000", connection.connected_pid);
    } else {
        my_printf("%s: missed\n\n", connection.last_get_string);
        navy->c_map2d[ypos][xpos] = 'o';
